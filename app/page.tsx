...
function Choice({ label, value, onChange, options }: {label:string; value:string; onChange:(v:string)=>void; options:[string,string][]}) {
  return <AppDropdown label={label} value={value} onChange={onChange} options={options.map(([optionValue,optionLabel])=>({value:optionValue,label:optionLabel}))}/>;
}

function skillOptions(skills:Skill[]):AppDropdownOption[] {
  return [...skills].sort((a,b)=>a.name.localeCompare(b.name)).map(skill=>({value:skill.name,label:skill.name,searchText:[skill.description,...skill.ranks.flatMap(rank=>[rank.name,rank.description])].filter(Boolean).join(' '),content:<span className="skill-option-text"><SkillName id={skill.id} passive/><small className="skill-option-snippet">{skill.description??skill.ranks[0]?.description}</small></span>}));
}

function SkillSearch({ value, onChange, skills, label='Skill' }: {value:string;onChange:(v:string)=>void;skills:Skill[];label?:string}) {
...
export default function Home() {
  ...
  const [charmBuilderReturn,setCharmBuilderReturn] = useState<CharmBuilderReturn>(null);
  ...
  const customCharmTemplate = useMemo(()=>catalog?.equipments.filter(e=>e.slot==='charm'&&e.random).sort((a,b)=>b.rarity-a.rarity)[0],[catalog]);
  ...
  const optimizedOtherSkills = useMemo(()=>{
    if(!optimizerResult||!optimizedSummary)return [];
    const selectedIds=new Set([...optimizerResult.skills.map(item=>item.skill.id),...optimizerResult.bonuses.map(item=>item.skill.id)]);
    return optimizedSummary.activeSkills.filter(item=>!selectedIds.has(item.skill.id));
  },[optimizerResult,optimizedSummary]);
  ...
  function openCharmBuilder(returnTo:CharmBuilderReturn=null) {
    const charm=build.charm?.equipment.random?build.charm.equipment:null;
    setCustomSkills(charm?.skills??[]);setCustomSlots(charm?.slots??[]);setCharmBuilderReturn(returnTo);
    if(returnTo==='picker')setPicker(null);
    if(returnTo==='optimizer')setOptimizerOpen(false);
    setCustomOpen(true);
  }
  function closeCharmBuilder() {
    setCustomOpen(false);
    if(charmBuilderReturn==='picker')setPicker({slot:'charm'});
    if(charmBuilderReturn==='optimizer')setOptimizerOpen(true);
    setCharmBuilderReturn(null);
  }
  function saveCharm() {
    const base=build.charm?.equipment.random?build.charm.equipment:customCharmTemplate;if(!base)return;
    const valid=customSkills.flatMap(ref=>{const skill=skillById.get(ref.id);const max=skill?Math.max(...skill.ranks.map(r=>r.level)):0;return skill&&ref.level>0?[{id:ref.id,level:Math.min(ref.level,max)}]:[];}).slice(0,3);
    const validSlots=customSlots.filter(s=>s.level>=1&&s.level<=3).slice(0,3);
    const updated={...base,skills:valid,slots:validSlots,random:true};
    setBuild(b=>equip(b,updated));setCustomOpen(false);if(charmBuilderReturn==='optimizer')setOptimizerOpen(true);setCharmBuilderReturn(null);setAnnouncement('Custom charm equipped.');
  }
  ...
  return <SkillDescriptions skills={catalog?.skills??[]}><div className="app-shell">
            ...
            return <div className={`equipment-row ${e?'is-equipped':''}`} key={slot}>
              ...
              <div className="equipment-copy"><div className="slot-label">{labels[slot]} {e&&<span className="rarity" style={rarityStyle(e.rarity)}>RARITY {e.rarity}</span>}</div>
                ...
                {e&&<><p className="equipment-subline">{isWeaponBuildSlot(slot)?`${weaponTypes[e.kind]}`:e.defense?`${e.defense[defenseMode]} defense`:e.random?'Custom charm · enter your roll':'Forged charm'}{e.skills.length>0&&<> · {skillText(e.skills)}</>}</p>{isWeaponBuildSlot(slot)&&<WeaponRowStats weapon={e}/>}
                  ...
                  {e.random&&<button className="text-button" onClick={()=>openCharmBuilder()}><SlidersHorizontal size={13}/> Edit charm skills & slots</button>}
    ...
    <Dialog open={!!picker} onOpenChange={open=>{if(!open)setPicker(null);}}><DialogContent className="equipment-dialog">
      ...
      {!targetSlot&&picker?.slot==='charm'&&<button className="secondary-button build-charm-button" type="button" onClick={()=>openCharmBuilder('picker')}><SlidersHorizontal size={14}/> Build my charm</button>}
    ...
    <AppModal
      open={customOpen}
      onOpenChange={open=>{if(!open)closeCharmBuilder();}}
      title="Build my charm"
      description="Add up to three skills at any level through their respective maximum, plus up to three decoration slots from level 1 to 3."
      footer={<><button className="primary-button" onClick={saveCharm}>Equip custom charm</button><button className="secondary-button" onClick={closeCharmBuilder}>Cancel</button></>}
    >
      <section className="app-modal-section"><h3>Skills</h3><div className="custom-skills">{[0,1,2].map(i=>{const ref=customSkills[i];const skill=skillById.get(ref?.id??-1);return <div key={i} className="custom-row"><SkillSearch label={`Charm skill ${i+1}`} skills={ordinarySkills.filter(s=>!customSkills.some((r,j)=>j!==i&&r.id===s.id))} value={skill?.name??''} onChange={name=>{const found=ordinarySkills.find(s=>s.name===name);setCustomSkills(rows=>{const next=[...rows];next[i]={id:found?.id??-1,level:found?1:0};return next;});}}/><Choice label={`Skill ${i+1} level`} value={String(ref?.level||1)} options={(skill?.ranks??[{level:1}]).map(r=>[String(r.level),`Lv. ${r.level}`])} onChange={v=>setCustomSkills(rows=>rows.map((r,j)=>i===j?{...r,level:+v}:r))}/></div>;})}</div></section>
      <section className="app-modal-section"><h3>Decoration slots</h3>{[0,1,2].map(i=><div className="custom-row" key={i}><Choice label={`Charm slot ${i+1}`} value={String(customSlots[i]?.level??0)} onChange={v=>setCustomSlots(rows=>{const next=[...rows];next[i]={kind:next[i]?.kind??'armor',level:+v};return next;})} options={[[ '0','No slot'],['1','Level 1'],['2','Level 2'],['3','Level 3']]}/><Choice label={`Slot ${i+1} type`} value={customSlots[i]?.kind??'armor'} options={[[ 'armor','Armor'],['weapon','Weapon']]} onChange={v=>setCustomSlots(rows=>{const next=[...rows];next[i]={level:next[i]?.level??0,kind:v as 'armor'|'weapon'};return next;})}/></div>)}</section>
    </AppModal>
    <AppModal
      open={optimizerOpen}
    ...
    >
      {!optimizerResult?<>
        ...
        <section className="optimizer-section optimizer-other-skills"><h3>Other skills <span>From the optimized equipment and decorations</span></h3>{optimizedOtherSkills.length?<table className="optimizer-skills"><thead><tr><th>Skill</th><th>Level</th><th>Max</th></tr></thead><tbody>{optimizedOtherSkills.map(item=><tr key={item.skill.id}><td><SkillName id={item.skill.id} passive/></td><td>{item.level}</td><td>{item.max}</td></tr>)}</tbody></table>:<p className="muted optimizer-hint">No additional skills came with this build.</p>}</section>
